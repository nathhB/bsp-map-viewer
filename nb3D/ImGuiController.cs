...
namespace nb3D
{
    ...
    {
        private static Keys[] s_keys = (Keys[])Enum.GetValues(typeof(Keys));
        ...
        private Dictionary<Keys, char> m_keysToSpecialChars = new Dictionary<Keys, char>()
        {
            { Keys.Space, ' ' },
            { Keys.Slash, '/' },
            { Keys.Backslash, '\\' },
        ...
        /// </summary>
        public ImGuiController(int width, int height, System.Numerics.Vector2 scaleFactor)
        {
            _windowWidth = (int)(width * scaleFactor.X);
            _windowHeight = (int)(height * scaleFactor.Y);
    ...
    outputColor = color * texture(in_fontTexture, texCoord);
        ...
        private void UpdateImGuiInput(GameWindow wnd)
        {
            ...
            foreach (var key in s_keys)
            {
                ...
                var isKeyDown = KeyboardState.IsKeyDown(key);

                io.AddKeyEvent(TranslateKey(key), isKeyDown);

                if (KeyboardState.IsKeyPressed(key) && TryGetCharFromInputKey(key, out var ch))
                {
                    PressedChars.Add(ch);
                }
            }
        ...
        private bool TryGetCharFromInputKey(Keys key, out char ch)
        {
            if (key >= Keys.A && key <= Keys.Z)
            {
                ch = (char)('a' + ((int)key - (int)Keys.A));
                return true;
            }

            if (key >= Keys.D0 && key <= Keys.D9)
            {
                ch = (char)('0' + ((int)key - (int)Keys.D0));
                return true;
            }

            if (key >= Keys.KeyPad0 && key <= Keys.KeyPad9)
            {
                ch = (char)('0' + ((int)key - (int)Keys.KeyPad0));
                return true;
            }
            ...
            if (m_keysToSpecialChars.TryGetValue(key, out var specialChar))
            {
                ch = specialChar;
                return true;
            }
